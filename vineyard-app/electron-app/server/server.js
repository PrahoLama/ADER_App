...
/**
 * Get annotations for an image (for editing)
 * GET /api/annotations/:imageId
 */
app.get('/api/annotations/:imageId', (req, res) => {
  const imageId = req.params.imageId;
  const annotationFile = path.join(annotationsDir, `${imageId}_annotations.json`);
...
app.get('/api/annotations', (req, res) => {
  try {
    const files = fs.readdirSync(annotationsDir)
      ...
      .map(f => {
        const data = JSON.parse(fs.readFileSync(path.join(annotationsDir, f), 'utf8'));
        return {
          id: f.replace('_annotations.json', ''),
          filename: f,
          image: data.image,
          detection_count: data.detection_count,
          status: data.status || 'auto_annotated',
          timestamp: data.timestamp
        };
      });
    res.json({ annotations: files });
  } catch (err) {
    res.status(500).json({ error: 'Failed to list annotations', details: err.message });
  }
});
 ...
 * Body: { detections: [...], status: 'corrected' }
 */
app.put('/api/annotations/:imageId', (req, res) => {
  ...
  try {
    ...
    // If file exists, read it; otherwise create new structure
    if (fs.existsSync(annotationFile)) {
      existingData = JSON.parse(fs.readFileSync(annotationFile, 'utf8'));
    } else {
      // Create new annotation file
      console.log(`📝 Creating new annotation file for ${imageId}`);
      existingData = {
        image: `${imageId}.JPG`,
        image_path: path.join(uploadDir, `${imageId}.JPG`),
    ...
    const updatedData = {
      ...existingData,
      detections: req.body.detections || existingData.detections,
      detection_count: (req.body.detections || existingData.detections).length,
      status: req.body.status || 'corrected',
      last_modified: new Date().toISOString(),
      manual_corrections: [
        ...(existingData.manual_corrections || []),
        {
          timestamp: new Date().toISOString(),
          action: 'edited',
          previous_count: existingData.detection_count || 0,
          new_count: (req.body.detections || existingData.detections).length
        }
      ]
    };
    
    fs.writeFileSync(annotationFile, JSON.stringify(updatedData, null, 2));
    
    console.log(`✏️ Annotations updated for ${imageId}: ${updatedData.detection_count} detections`);
    res.json({ success: true, data: updatedData });
  } catch (err) {
    console.error('Failed to save annotations:', err);
    res.status(500).json({ error: 'Failed to save annotations', details: err.message });
  }
});

/**
 * Export annotated image with bounding boxes drawn
 * POST /api/annotations/:imageId/export
 * Body: { format: 'png'|'yolo_txt'|'json' }
 */
app.post('/api/annotations/:imageId/export', async (req, res) => {
  const imageId = req.params.imageId;
  ...
  try {
    const data = JSON.parse(fs.readFileSync(annotationFile, 'utf8'));
    const imagePath = data.image_path || path.join(uploadDir, data.image);
    ...
    if (format === 'json') {
    ...
    } else if (format === 'yolo_txt') {
      // Export in YOLO txt format (class x_center y_center width height)
      // Assuming image dimensions from annotation data
      const yoloLines = data.detections.map(det => {
        const normalized = det.bbox_normalized || {};
        const classId = det.class_id || 0;
        return `${classId} ${normalized.x_center || 0} ${normalized.y_center || 0} ${normalized.width || 0} ${normalized.height || 0}`;
      });
      
      const txtFilename = `${imageId}.txt`;
      const txtPath = path.join(annotationsDir, txtFilename);
      ...
      // Use Python to draw boxes on image
      ...
      await new Promise((resolve, reject) => {
        pythonProcess.on('close', (code) => {
          ...
          if (fs.existsSync(detectionsFile)) {
            fs.unlinkSync(detectionsFile);
          }
          
          if (code === 0 && fs.existsSync(outputPath)) {
            resolve();
          } else {
            reject(new Error(pythonError || 'Failed to draw annotations'));
          }
        });
      });
          ...
          if (code === 0) {
            try {
              ...
              const annotationData = {
                ...
                image_size: detection.image_size || { width: 4000, height: 3000 },
...
/**
 * Download all modified/corrected annotations and images as ZIP
 * GET /api/download-modified
 */
app.get('/api/download-modified', async (req, res) => {
  const archiver = require('archiver');
  
  try {
    // Find all corrected annotation files
    const annotationFiles = fs.readdirSync(annotationsDir)
      .filter(f => f.endsWith('_annotations.json'));
    
    const modifiedAnnotations = annotationFiles.filter(f => {
      try {
        const data = JSON.parse(fs.readFileSync(path.join(annotationsDir, f), 'utf8'));
        return data.status === 'corrected' || data.status === 'manually_reviewed' || 
               (data.manual_corrections && data.manual_corrections.length > 0);
      } catch {
        return false;
      }
    });
    ...
    if (modifiedAnnotations.length === 0) {
      return res.status(404).json({ error: 'No modified annotations found' });
    }
    
    console.log(`📦 Preparing download of ${modifiedAnnotations.length} modified annotations`);
    
    // Set up ZIP response
    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Disposition', `attachment; filename=modified_annotations_${Date.now()}.zip`);
    
    const archive = archiver('zip', { zlib: { level: 9 } });
    archive.pipe(res);
    
    // Add each modified annotation and its image
    for (const annotationFile of modifiedAnnotations) {
      const annotationPath = path.join(annotationsDir, annotationFile);
      const data = JSON.parse(fs.readFileSync(annotationPath, 'utf8'));
      
      // Add JSON file
      archive.file(annotationPath, { name: `annotations/${annotationFile}` });
      ...
      try {
        ...
        if (fs.existsSync(imagePath)) {
          ...
          if (fs.existsSync(outputPath)) {
            archive.file(outputPath, { name: `annotated_images/annotated_${imageName}` });
          }
        }
      } catch (err) {
        console.warn(`⚠️ Could not generate annotated image for ${imageName}:`, err.message);
      }
    }
